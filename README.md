...
* **🗣️ 対応言語:** 日本語、英語、中国語、韓国語、スペイン語、ロシア語、フランス語、ドイツ語、ポルトガル語、タイ語に完全対応。

### ⚠️ 重要なお知らせ：APIキーは必須です
正直にお伝えしますと、**SyncVRCは翻訳サービス（Gemini、DeepL、OpenAIなど）のAPIキーなしでは動作しません**。本ソフトウェアを使用するには、ご自身でAPIキーを取得していただく必要があります。
可能であれば、**有料のAPIキー**を使用することを強くお勧めします。無料枠のキーには厳しい制限（レートリミット）があることが多く、連続した会話の中で翻訳が止まったり、スキップされたりして、非常にストレスを感じる原因になります。取得方法の詳細はこちら：[AIプロバイダーとAPI設定](https://finalsiren1.github.io/SyncVRC/ai-providers)
...
* **🗣️ 支持语言:** 全面支持中文、英文、日文、韩文、西班牙文、俄文、法文、德文、葡萄牙文和泰文。

### ⚠️ 重要提示：必须提供 API 密钥
为了完全透明，**如果没有翻译服务（如 Gemini、DeepL 或 OpenAI）的 API 密钥，SyncVRC 将无法运行**。您必须获取自己的 API 密钥才能使用本软件。
如果可能的话，我们强烈建议您使用**付费 API 密钥**。免费层级的密钥通常有严格的频率限制，这会导致翻译在连续对话中暂停、跳过或失败，从而带来令人沮丧的体验。获取方法详情请见：[AI 提供商与 API 设置](https://finalsiren1.github.io/SyncVRC/ai-providers)
...
* **🗣️ 지원 언어:** 한국어, 영어, 일본어, 중국어, 스페인어, 러시아어, 프랑스어, 독일어, 포르투갈어, 태국어를 완벽 지원합니다.

### ⚠️ 중요 안내: API 키 필수
솔직히 말씀드리면, **SyncVRC는 번역 서비스(Gemini, DeepL, OpenAI 등)를 위한 API 키 없이는 작동하지 않습니다**. 이 소프트웨어를 사용하려면 본인의 API 키를 직접 발급받아야 합니다.
가능하다면 **유료 API 키**를 사용하는 것을 강력히 권장합니다. 무료 등급 키는 엄격한 사용량 제한이 있어 대화 중에 번역이 멈추거나, 끊기거나, 실패하여 매우 답답한 경험을 할 수 있습니다. 발급 방법은 다음을 참조하세요: [AI 제공업체 및 API 설정](https://finalsiren1.github.io/SyncVRC/ai-providers)
...
* **🗣️ Fully Supported Languages:** English, Japanese, Chinese, Korean, Spanish, Russian, French, German, Portuguese, and Thai.
...
We highly recommend using a **Paid API Key** if possible. Free tier keys often have strict rate limits which will cause translations to pause, skip, or fail during continuous conversation, leading to a frustrating experience. For details: [AI Providers & API Setup](https://finalsiren1.github.io/SyncVRC/ai-providers)
